...
Pipeline results are written to the directory specified by `--outdir`. The output is organized by analysis stage:
...
The `preprocessing/` directory contains read quality-control results and intermediate FASTQ files generated before DADA2 denoising.
...
* `qc_raw/` contains per-sample SeqKit statistics for the input reads before filtering and primer trimming.
* `filtered_input_FASTQ/` contains the FASTQ files after quality and length filtering, primer trimming, and optional downsampling. These reads are used as input for DADA2.
* `cutadapt/` contains the per-sample Cutadapt log files generated during primer trimming.
* `qc_post_trim/` contains per-sample SeqKit statistics after preprocessing.
* `reads_QC/` contains combined read-quality statistics across all samples and a summary of primer trimming.

### DADA2

The `dada2/` directory contains intermediate and final results from ASV inference.
...
* `filter_stats/` contains per-sample statistics from DADA2 read filtering.
* `error_model/` contains the estimated DADA2 error model (`errorfun.rds`), the reads used for error-model estimation, and a diagnostic error-model plot (`plot_error_model.pdf`).
* `denoised/` contains the per-sample DADA2 denoising objects (`*.dada.rds`).
* `seqtab/seqtab.rds` contains the ASV sequence table before chimera removal.
* `chimera_removal/seqtab_nochim.rds` contains the sequence table after chimera removal. `chimera_stats.tsv` summarizes the effect of chimera removal.
* `filtered_asvs/` contains the final filtered ASVs. `dada2_ASV.fasta` contains the ASV sequences, `dada2_table_filtered.tsv` contains their abundances across samples, and `filter_asv_stats.tsv` summarizes abundance-based ASV filtering.

### Taxonomy

The `taxonomy/` directory contains classifier-specific taxonomic assignment results.
...
`nb_tax/` contains results from the naive Bayesian classifier. Depending on the selected databases, this includes individual database assignments as well as the combined best assignment:

* `*_nb.tsv` — taxonomic assignments for an individual naive Bayesian database.
* `best_nb_taxonomy_withDB.tsv` — selected best assignment together with the database from which the assignment originated.
* `best_nb_taxonomy.tsv` — selected best taxonomic assignment used for the final output.

`vsearch_tax/` contains intermediate results from VSEARCH-based classification:

* `*.vsearch_blast6.tsv` — VSEARCH alignment results in BLAST6-like format.
* `*.vsearch_hits.tsv` — hits retained for taxonomic assignment.
* `*.vsearch_lca.tsv` — taxonomic assignments obtained using the VSEARCH LCA procedure.

The exact files present depend on the taxonomic databases and classifiers enabled for the run.

### Final results

The `final/` directory contains the main files intended for downstream analysis and reporting.
...
├── best_nb_tax_merged_freq_tax.tsv
├── best_nb_taxonomy_assignment_summary.tsv
├── best_nb_taxonomy_deepest_rank_summary.tsv
├── vsearch_tax_merged_freq_tax.tsv
├── vsearch_taxonomy_assignment_summary.tsv
└── vsearch_taxonomy_deepest_rank_summary.tsv
```

* `frequency.tsv` — final ASV abundance table.
* `dada2_tracking.tsv` — read counts retained through the different DADA2 processing steps.
* `rarefaction_depth_suggested.txt` — suggested rarefaction depth based on the final abundance data.
* `*_tax_merged_freq_tax.tsv` — final ASV table combining ASV identifiers, sequences, taxonomic assignments, assignment confidence, and per-sample abundances.
* `*_taxonomy_assignment_summary.tsv` — cumulative taxonomic assignment statistics for Domain, Phylum, Class, Order, Family, Genus, and Species. The table reports both the number and percentage of ASVs assigned at each rank and the corresponding read-weighted abundance.
* `*_taxonomy_deepest_rank_summary.tsv` — distribution of the deepest taxonomic rank assigned to each ASV, reported both by number of ASVs and by read abundance.

Files beginning with `best_nb_` are generated from the naive Bayesian classification workflow, whereas files beginning with `vsearch_` are generated from the VSEARCH classification workflow. Only outputs for classifiers enabled in a particular run are produced.

### Pipeline information

The `pipeline_info/` directory contains Nextflow execution metadata:

* `execution_report.html` — Nextflow execution report including process resource usage.
* `execution_timeline.html` — timeline of process execution.
* `execution_trace.txt` — tabular execution trace containing task-level runtime and resource information.

These files are primarily useful for troubleshooting, reproducibility, and evaluating pipeline performance.

# Reference databases

The workflow supports taxonomic classification of PacBio HiFi amplicon sequence variants (ASVs) using marker-specific reference databases.

The reference database should be selected according to the sequenced marker. For bacterial and archaeal 16S rRNA data, the workflow supports SILVA, GTDB, and Greengenes2. Eukaryotic 18S, 28S, ITS, and long SSU–ITS–LSU amplicons are classified using the corresponding EUKARYOME reference database.

## Supported databases

| Database            | Workflow identifier | Marker         | Naive Bayes | VSEARCH-LCA |
| ------------------- | ------------------- | -------------- | :---------: | :---------: |
| SILVA 138.2         | `silva`             | 16S rRNA       |      ✓      |      —      |
| GTDB R220           | `gtdb`              | 16S rRNA       |      ✓      |      —      |
| Greengenes2 2024.09 | `gg2`               | 16S rRNA       |      ✓      |      —      |
| EUKARYOME SSU       | `euk_ssu`           | 18S / SSU rRNA |      ✓      |      ✓      |
| EUKARYOME LSU       | `euk_lsu`           | 28S / LSU rRNA |      ✓      |      ✓      |
| EUKARYOME ITS       | `euk_its`           | ITS            |      ✓      |      ✓      |
| EUKARYOME long      | `euk_long`          | SSU–ITS–LSU    |      ✓      |      ✓      |

In practice, database selection follows the sequenced marker:
...
Three reference databases are available for bacterial and archaeal 16S rRNA amplicons.

**SILVA** provides curated small- and large-subunit ribosomal RNA sequences across all domains of life. The workflow currently uses the **SILVA 138.2** reference.

**GTDB** provides a standardized genome-based taxonomy for Bacteria and Archaea. The workflow uses an officially published DADA2-compatible SSU reference derived from **GTDB release R220**.

**Greengenes2** integrates microbial genome and marker-gene information into an updated reference taxonomy. The workflow currently uses the officially published **Greengenes2 2024.09** DADA2-compatible reference.

These three databases are currently supported by the Naive Bayes classification workflow only. The pipeline deliberately does not generate its own VSEARCH/SINTAX versions of these databases.

## EUKARYOME reference databases

Eukaryotic amplicons are classified using the **EUKARYOME** reference database corresponding to the sequenced ribosomal marker:

* `euk_ssu` for 18S / SSU rRNA amplicons
* `euk_lsu` for 28S / LSU rRNA amplicons
* `euk_its` for ITS amplicons
* `euk_long` for long amplicons spanning the SSU–ITS–LSU region

EUKARYOME provides classifier-specific reference files. The workflow uses the official DADA2-formatted references for Naive Bayes classification and the official SINTAX-formatted references for VSEARCH-LCA classification. Both classification methods are therefore supported for all four EUKARYOME marker sets.

For example, the SSU database is organized as:
...
The same structure is used for the LSU, ITS, and long references:
